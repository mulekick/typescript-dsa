...
import {getRandomChar, timeExecution, benchmarkQueue, benchmarkStack, createArray} from "../helpers.ts";
...
try {
    ...
    describe(`===== SINGLY LINKED LIST BASED QUEUE O(1) =====`, () => {
        ...
        describe.each(inputSizes)(`input: set of %i unicode basic latin characters`, (x: number): void => {
            ...
            it(`queued ${ String(x) } characters and dequeued ${ String(Math.floor(x / 2)) } characters in ${ String(time) }ms`, (): void => {
    ...
    describe(`===== SINGLY LINKED LIST BASED STACK O(1) =====`, () => {
        ...
        describe.each(inputSizes)(`input: set of %i unicode basic latin characters`, (x: number): void => {
            ...
            // call function with current input, log result
            it(`pushed ${ String(x) } characters and popped ${ String(Math.floor(x / 2)) } characters in ${ String(time) }ms`, (): void => {
                expect(true).toBe(true);
            });
        });
    });
